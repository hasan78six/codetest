...
class BookingRepository extends BaseRepository
{
     ...
     * 
     * Contructor is there to intialize variabless
     */
     ...
     * @return array
     * 
     * This function helps to get jobs related to specific customer or translator
     */
    public function getUsersJobs($user_id)
    {
        ...
        $output = ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'cuser' => $cuser, 'usertype' => $usertype];

        return empty($cuser) ? null : $output;
    }
     ...
     * @return array
     * 
     * This function helps to get user job history
     */
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if (!empty($cuser)) {
            switch(true){
                case $cuser->is('customer'):
                    $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback', 'distance')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate($pagenum);
                    $usertype = 'customer';
                    break;
                case $cuser->is('translator'):
            ...
            case env('CUSTOMER_ROLE_ID'):
                ...
                switch(true){
                    ...
                    case in_array('normal', $data['job_for']) && in_array('certified_in_law', $data['job_for']):
                        ...
                        break;
                    case in_array('normal', $data['job_for']) && in_array('certified_in_health', $data['job_for']):
                ...
                $data['job_for'][] = isset($job->gender) ? ($job->gender == "male" ? "Man" : "Kvinna") : null;
                if (!empty($job->certified) && ($job->certified != "both") && ($job->certified != 'yes')) 
                {
                ...
                switch(true){
                    case !isset($data['from_language_id']):
                        $response['status'] = 'fail';
                        $response['message'] = "Du måste fylla in alla fält";
                        $response['field_name'] = "from_language_id";
                        break;
                    case $data['immediate'] == 'no':
                        ...
                        switch(true){
                            case (!isset($data['due_date']) && empty($data['due_date'])):
                                $response['status'] = 'fail';
                                $response['message'] = "Du måste fylla in alla fält";
                                $response['field_name'] = "due_date";
                                break;
                            case (!isset($data['due_time']) && empty($data['due_time'])):
                                $response['status'] = 'fail';
                                $response['message'] = "Du måste fylla in alla fält";
                                $response['field_name'] = "due_time";
                                break;
                            case (!isset($data['customer_phone_type']) && empty($data['customer_phone_type'])):
                                $response['status'] = 'fail';
                                $response['message'] = "Du måste göra ett val här";
                                $response['field_name'] = "customer_phone_type";
                                break;
                            case (!isset($data['duration']) && empty($data['duration'])):
                                $response['status'] = 'fail';
                                $response['message'] = "Du måste fylla in alla fält";
